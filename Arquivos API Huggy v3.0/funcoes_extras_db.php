...
function verificaHorarioSuporte()
{
    ...
    $dataSistem = 	gmdate('d/m/Y',time() + 3600*($timezone+date("I")));                // gmdate('F d Y H:i:s', time() + 3600*($timezone+date("I")));
    $horaSistem = gmdate('H:i:s',time() + 3600*($timezone+date("I")));                  // gmdate('F d Y H:i:s', time() + 3600*($timezone+date("I")));
    $diaSemana = gmdate('w',time() + 3600*($timezone+date("I")));                       // gmdate('F d Y H:i:s', time() + 3600*($timezone+date("I")));
    ...
    if (($diaSemana >= 1) && ($diaSemana <= 5)){                                        // Verifica se o dia atual está entre Segunda-feira e Sexta-feira
        foreach($diasFeriados as &$fer){                                                // Percorre o array com os feriados nacionais
            if ($fer == $dataSistem){
                $feriado = 1;                                                           // Data atual é feriado
                echo $fer . " = " . $dataSistem . "\n";
            }
        }
        ...
        foreach($diasFeriadosM as &$ferM){                                              // Percorre o array com os feriados municipais
            if ($ferM == $dataSistem){
                $feriado = 2;                                                           // Data atual é feriado
                echo $fer . " = " . $dataSistem . "\n";
            }
        }
        ...
        if ($feriado == 1){
            if (($horaSistem > $horaLimite1fer) && ($horaSistem < $horaLimite3fer)) {   // Verificando se está dentro do horário de atendimento
                ...
                return true;
                // horario q tem atendimento telefonico no feriado
            } else {
                echo "NÃO tem atendimento e é feriado Nacional";
                return false
                // horario q NÃO tem atendimento telefonico no feriado
            }
        } else if ($feriado == 2) {
            if (($horaSistem > $horaLimite2) && ($horaSistem < $horaLimite2fer)) {      // Verificando se está dentro do horário de atendimento
                ...
                return true;
                // horario q tem atendimento telefonico
            } else {
                echo "NÃO tem atendimento e não é feriado Municipal";
                return false;
                // horario q NÃO tem atendimento telefonico
            }
        } else {
        ...
        if ($feriado == 1){
            if (($horaSistem > $horaLimite1fer) && ($horaSistem < $horaLimite3fer)) {   // Verificando se está dentro do horário de atendimento
                echo "Tem atendimento e é feriado Nacional e Sábado";
                return true;
                // horario q tem atendimento telefonico no feriado
            } else {
                echo "NÃO tem atendimento e é feriado Nacional e Sábado";
                return false
                // horario q NÃO tem atendimento telefonico no feriado
            }
        } else if ($feriado == 2) {
            if (($horaSistem > $horaLimite2) && ($horaSistem < $horaLimite2fer)) {      // Verificando se está dentro do horário de atendimento
                echo "Tem atendimento e é feriado Municpal e Sábado";
                return true;
                // horario q tem atendimento telefonico
            } else {
                echo "NÃO tem atendimento e não é feriado Municipal e Sábado";
                return false;
                // horario q NÃO tem atendimento telefonico
            }
        } else {
            if (($horaSistem > $horaLimite2) && ($horaSistem < $horaLimite2fer)) {         // Verificando se está dentro do horário de atendimento
                echo "Tem atendimento e não é feriado e Sábado";
                return true;
                // horario q tem atendimento telefonico
            } else {
                echo "NÃO tem atendimento e não é feriado e Sábado";
                return false;
                // horario q NÃO tem atendimento telefonico
            }
        }
 ...
 * Segunda-feira à Sábado de 08:00:00 às 22:00:00
...
function verificaHorarioPagamentos()
{
    ...
    if (($diaSemana >= 1) && ($diaSemana <= 6)){                                        // Verifica se o dia atual está entre Segunda-feira e Sábado
