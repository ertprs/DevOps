<?php 
    ...
    // Busca na tabela a informação do flowToken tipo TRANSFER_CRM_BLOQUEIO 
    $FlowToken = "SELECT chatID,dataCriacao FROM startedAutomationFlow WHERE flowToken LIKE '%TRANSFER_CRM_BLOQUEIO%' AND checking = '0' ORDER BY dataCriacao DESC LIMIT 1";//  LIMIT 1";
    ...
    if (empty(mysqli_num_rows($Resultado))) {                                                               // Verificando se o retorno está vazio
        echo "<script type='text/javascript'>console.log('Observação: Não foi encontrado nenhum Flow do Tipo TRANSFER_CRM_BLOQUEIO.');</script>";
    } else {
        ...
        if (($cpf == NULL) && ($cnpj == NULL)) {
            ...
            for ($i=0; $i<10; $i++) {
                ...
                if ((is_numeric($body)) && ($bodyTam == 1)) {                                               // Verificando se body é um número e se tem tamanho = 1
                    if (isset($servicos[$body - 1])) {                                                      // Verificado se o serviço existe no array criado anteriormente, array inicia com '0'
                        ...
                        // Verificar no CRM se tem Bloqueio por Pagamentos, Cancelamento ou Bloqueio por Contratos
                        // opção 1 Bloqueio por Pagamentos | 2 Bloqueio de Contratos | 3 Serviçao Cancelado
                        $servicoEscolhido = $servicos[$body - 1];
                        $opcao = 2;

                        if ($opcao == 1){                                                                   // Opção para o serviço bloqueado pelo Pagamentos
                            $parametros = array(
                                "text"=> "O serviço $servicoEscolhido se encontra com Bloqueio pelo Setor de Pagamentos.",
                                "isInternal"=> false
                            ); 
                            enviaMensagemHuggy($url,$parametros,$type);
                            sleep(5);
                            ...
                            $bloqueio = 1;                                                                  // Será dado opção ao cliente do APP ou falar com Atendente
                            $i = 10;
                        } else if ($opcao == 2){                                                            // Opção para o serviço bloqueado pelo Contratos
                            $parametros = array(
                                "text"=> "O serviço $servicoEscolhido se encontra com Bloqueio pelo Setor de Contratos. \nAguarde um momento, por favor! Você será transferido a um de nossos Atendentes.",
                                "isInternal"=> false
                            ); 
                            enviaMensagemHuggy($url,$parametros,$type);
                            sleep(5);
                            
                            // Transfere cliente para fila de atendimento do Setor de Contratos
                            // Passo 1 - Definir o departamento
                            $url1 = "chats/$chatId/department";
                            $parametros1 = array( "department"=> "11743" );                                 // Id(código) departamento do Setor de Contratos
                            $type1 = "PUT";
                            /*enviaMensagemHuggy($url1,$parametros1,$type1);*/
                            ...
                            $bloqueio = 3;                                                                  // Finalizando a transferencia para Setor de Contratos
                            $i = 10;
                        } else if ($opcao == 3){                                                            // Opção para o serviço Cancelado
                            $parametros = array(
                                "text"=> "O serviço $servicoEscolhido se encontra cancelado.",
                                "isInternal"=> false
                            ); 
                            enviaMensagemHuggy($url,$parametros,$type);
                            sleep(5);
                            
                            $bloqueio = 0;                                                                  // Será transferido cliente para falar com Atendente
                            $i = 10;
                        } 
            ...
            if ($bloqueio == 1) {                                                                          // Verificação de quando o cliente está bloqeuado pelo Pagamentos
                // Inicio do Loop com 100x e tempo de espara 2s para poder coletar
                for ($i=0; $i<10; $i++) {
                    // Busca na tabela a ultima informação enviada pelo cliente com a opção desejada
                    $BuscaPostagem = "SELECT body,dataCriacao FROM receivedAllMessage WHERE chatID = $chatId AND dataCriacao > $dataCriacao ORDER BY dataCriacao DESC LIMIT 1";
            ...
            } else if ($bloqueio == 0){                                                                     // Se o cliente não digitou nada ou digitou algum texto, será transferido
                $parametros = array(
                    "text"=> "Aguarde um momento, por favor! Você será transferido a um de nossos Atendentes.",
                    "isInternal"=> false
                ); 
                ...
                // Passo 1 - Definir o departamento
                $url1 = "chats/$chatId/department";
                $parametros1 = array( "department"=> "11632" );
                $type1 = "PUT";
                /*enviaMensagemHuggy($url1,$parametros1,$type1);*/
        ...
        // Atualizando na tabela a informação do flowToken tipo TRANSFER_CRM_BLOQUEIO 
